...
## load data
#D_train = np.loadtxt('hw3_train.txt')
#Label_train = D_train[:,-1]
#Data_train = D_train[:,:-1]
#
#D_test = np.loadtxt('hw3_train.txt')
#Label_test = D_test[:,-1]
#Data_test = D_test[:,:-1]
#
#
#cls = logistic_SGD(Data_train,Label_train)
#cls.train()
#cls.predicting_error(Data_test,Label_test)


#plt.scatter(X[y==1,0],X[y==1,1],c = 'r',marker='x')
#plt.scatter(X[y!=1,0],X[y!=1,1],c = 'b',marker='o')
#plt.plot([0, 8 ], [ -cls.theta[0]/cls.theta[2] , (cls.theta[0]+cls.theta[1]*8)/(-cls.theta[2])  ],'k--')


#plt.show()
## load data
#Data = np.loadtxt('hw3_train.txt')
#Label_train = Data[:,-1]
#Data_train = Data[:,:-1]
#
#
#Dtest = np.loadtxt('hw3_test.txt')
#ytest = np.copy(Dtest[:,-1])
#ytest = np.reshape(ytest,[len(ytest),1])
#Xtest = np.append(np.ones([len(ytest),1]),Dtest[:,:-1],axis=1)
#
#
## training
#cls_logistics_newton = logistic_Newton(Data_train,Label_train)
#cls_logistics_newton.train()
#cls_logistics_newton.predicting_error(Xtest,ytest)
